...
export function currentTagSelection(window: MainWindow): string[] {
  const pane = mainPane(window);
  // CollectionViewItemTree owns the actual tag predicate through its active rows.
  // Prefer those rows over tag-selector presentation state or a possibly stale
  // singular collection-tree row.
